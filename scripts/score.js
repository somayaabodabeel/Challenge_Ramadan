...
export class Score {
    ...
    async getHighScore() {
        ...
        // Store a reference to the local persistence storage
        const localPersistence = Persistence.local;

        try {
            // Fetch saved data (if any)
            const result = await localPersistence.get('score');
    ...
    async saveHighScore() {
        ...
        if (score < this.highScore) {
            return; // score wasn't a high score, bail
        }
        ...
        try {
            // Store the data
            await localPersistence.set('score', data);
        } catch (ex) {
            // console.log(ex);
        }
    }
